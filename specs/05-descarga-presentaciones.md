...
- Apertura de archivos vía `shell.openPath()` de Electron según la extensión real del archivo descargado, reemplazando la lógica actual de `open-presentation` que solo invoca `POWERPNT.EXE`. Esto cubre `.ppt`/`.pptx`/`.pdf`/etc. usando la app por defecto del sistema operativo.
...
5. En `main.js`, reescribir el handler `open-presentation`: recibe `presentacion` (no `pptPath`); si `isDownloaded(presentacion)` es `true`, resuelve la ruta local y abre directo; si no, llama a `downloadPresentacion` (descarga on-demand, usando `inFlightIds` para no duplicar si ya está en cola background) y, si tiene éxito, abre el archivo recién descargado. Reemplazar la apertura vía `POWERPNT.EXE`/`execFile` por `shell.openPath(rutaLocal)`. Verificación manual: con una presentación no descargada aún, click en Go dispara la descarga on-demand (visible en logs/Network) y luego abre el archivo con la app por defecto del SO.
...
- [ ] En Speaker, al hacer click en "Go" sobre una presentación ya descargada y al día, el archivo se abre con la app por defecto del sistema operativo según su extensión (no solo PowerPoint).
...
- **Sí:** apertura de archivos vía `shell.openPath()` según extensión real, reemplazando la lógica específica de PowerPoint de SPEC 02. Generaliza la apertura a cualquier extensión que la API real está devolviendo (`.pdf` confirmado, no solo `.pptx`), delegando en el SO qué programa usar.
